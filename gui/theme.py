"""theme.py — zentrales Lila-Glas-Farbschema für Umzugstool.
...
Fortschrittsbalken, Scrollbalken, Menüs, Dialoge - im selben dezenten
...
Zentrale Anlaufstelle: Farbe hier ändern -> wirkt in der ganzen App.
"""
...
ACCENT_LIGHT = "#8c68dc"   # heller Akzent (Ränder, Kontrast)
CHECK_MARK = "#f2ecfb"     # fast weiß (Haken, Schalterknauf)
TEXT_MUTED = "#9c8fc4"     # gedämpfter Lila-Grauton (Branding, Fußzeile)
CORNER = 16                # Eckenradius für das Content-Panel


def apply_purple_theme() -> None:
    """Setzt Dark-Mode + kompakte Skalierung und färbt das Theme lila um.
    ...
    # Widget-/Schriftgröße wird in app.py bildschirmabhängig gesetzt
    # (kompakt auf Laptops, größer auf 4K-Monitoren).

    theme = ctk.ThemeManager.theme

    def both(color: str) -> list:
        """Gleiche Farbe für Hell-/Dunkel-Modus (App ist auf Dark fixiert)."""
        return [color, color]

    def override(widget: str, **colors) -> None:
        """Setzt nur Farbschlüssel, die im geladenen Theme wirklich existieren
