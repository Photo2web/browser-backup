"""
blacklist.py — zentrale Cache-/Ausschluss-Definition für die Sicherung.

Herkunft dieser Werte: Phase 0 (siehe PHASE0_NOTIZEN.md, Abschnitt
"Finale Blacklist"). Zwei reale Messläufe von inspect_profiles.py auf
...
Es gibt zwei Arten von Einträgen:
  ...
  - "patterns": Pfadmuster (glob, über fnmatch) relativ zum Profilordner,
             für Fälle wie "Service Worker/CacheStorage", die eine
             Ebene tiefer liegen.
...
Große, aber nicht eindeutig cache-benannte Ordner (WebStorage,
...
# in aktuellen Versionen nach %LOCALAPPDATA% aus, außerhalb des hier
# gesicherten Roaming-Profils). Sie bleiben trotzdem in der Blacklist, als
# wirkungsloser Kompatibilitäts-Eintrag für ältere Firefox-Versionen, die
# noch alles im Roaming-Profil halten.
FIREFOX_NAMES = frozenset(
    {
        "cache2",
        "startupCache",
        "shader-cache",       # bestätigt vorhanden, 693 KB
        "thumbnails",
        "OfflineCache",
        "crashes",            # bestätigt vorhanden, 66 B
        "minidumps",           # bestätigt vorhanden, 0 B
        "datareporting",       # bestätigt vorhanden, 6,3 MB
        "lock",
        ".parentlock",
        "parent.lock",         # bestätigt vorhanden, 0 B
    }
)

# Bestätigt per Drilldown: storage/<default|permanent|temporary>/<origin>/cache
...
# Bleiben als wirkungsloser Kompatibilitäts-Eintrag für ältere Versionen.
CHROMIUM_NAMES = frozenset(
    {
        "Cache",               # bestätigt vorhanden, bis 330,7 MB
        "Code Cache",          # bestätigt vorhanden, bis 221,3 MB
        "GPUCache",            # bestätigt vorhanden, bis 5,6 MB
        "GrShaderCache",
        "ShaderCache",
        "Media Cache",
        "Application Cache",
...
# Bestätigt per Drilldown: "Service Worker/CacheStorage" ist der dominante
...
# bleibt bewusst erhalten — ist für die Funktion der Service Worker nötig.
CHROMIUM_PATTERNS = (
...
def get_blacklist(browser_key: str) -> tuple[frozenset, tuple]:
    """Gibt (names, patterns) für den angegebenen Browser-Key zurück."""
...
def is_excluded(relative_posix_path: str, browser_key: str) -> bool:
    """Prüft, ob ein Pfad (relativ zum Profilordner, posix-Schreibweise
    mit '/' als Trenner) von der Sicherung ausgeschlossen werden soll.

    - Top-Level-Namen (erstes Pfadsegment) werden gegen 'names' geprüft.
    - Der komplette relative Pfad wird zusätzlich gegen 'patterns' (glob)
      geprüft, für tiefer liegende Cache-Ordner wie 'Service Worker/CacheStorage'.
    """
...
def excluded_patterns_for_manifest(browser_key: str) -> list[str]:
    """Liste aller Blacklist-Einträge (Namen + Muster) für das Manifest
